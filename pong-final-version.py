...
class Paddle(): # player, left paddle
    def __init__(self):
    ...
    def draw(self, isBot):
        if isBot == True:
            arcade.draw_rectangle_filled((SCREEN_WIDTH - PADDLE_SPACING), self.y, self.width, self.height, self.color)
        else:
            arcade.draw_rectangle_filled(PADDLE_SPACING, self.y, self.width, self.height, self.color)
...
class Ball():
    ...
    def update(self, leftpaddle, rightpaddle, delta_time):
        ...
        if self.x <= (PADDLE_SPACING + (leftpaddle.width / 2) + self.size) and self.y >= leftpaddle.y - (leftpaddle.height / 2)  and self.y <= leftpaddle.y + (leftpaddle.height / 2):
            ...
            while self.x <= (PADDLE_SPACING + (leftpaddle.width / 2) + self.size):
                self.x += 1

            self.delta_x = -self.delta_x

        if self.x >= ((SCREEN_WIDTH - PADDLE_SPACING) - (rightpaddle.width / 2) - self.size) and self.y >= rightpaddle.y - (rightpaddle.height / 2) and self.y <= rightpaddle.y + (rightpaddle.height /2):
            while self.x >= ((SCREEN_WIDTH - PADDLE_SPACING) - (rightpaddle.width / 2) - self.size):
                self.x -= 1
...
class GameView(arcade.View): # view for the game
    ...
    def on_draw(self):
        ...
        self.ball.draw()
        self.leftpaddle.draw(False)
        self.rightpaddle.draw(True)
...
class PauseView(arcade.View): # view for pause screen. if esc is pressed, this view will show (it renders the gameview too)
    ...
    def on_draw(self):
        ...
        self.game_view.leftpaddle.draw(False) # render paused leftpaddle
        self.game_view.rightpaddle.draw(True) # render paused rightpaddle
